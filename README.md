...
- ✦ 매일 감사 노트 기록 (최대 5가지)
- 🏷 감사 태그 (가족/건강/일/신앙/관계/나 자신) + 태그별 통계
- 🔍 기록 검색
- 📈 월간 감정 흐름 차트
- 🖼 감사 카드 이미지 공유
- 🙏 기도노트 (기도 중 / 응답받음 / Firebase 동기화)
- 🌿 그룹 피드 공유 (이름/익명 선택, 기간·작성자 필터, ❤️ 리액션)
- 📅 챌린지 (7일 → 21일 → 30일)
- 🔔 매일 리마인더 알림 (Service Worker)
- 🔒 PIN 잠금 (로컬 기기 보호)
- 🎨 테마 / 폰트 / 배경 패턴 커스터마이징
- ☁️ Firebase 클라우드 동기화 (감사기록 + 기도 + 그룹 피드, 오프라인 재시도 큐 포함)
...
### v3.24 · 2026.07.04
- 🔍 기록 검색 (감사 내용/메모 키워드 검색)
- 🖼 감사 카드 이미지로 공유 (Web Share API / 다운로드)
- ☁️ 오프라인 쓰기 재시도 큐 (네트워크 복귀 시 자동 재동기화)
- 🏷 감사 태그/카테고리 (가족·건강·일·신앙·관계·나 자신) + 태그별 통계
- 📈 월간 감정 흐름 차트
- ❤️ 그룹 피드 리액션
- 🔒 PIN 잠금 (로컬 기기 전용, 서버 인증 아님)

---

### v3.20 ~ v3.23
- 그룹 피드 공유 기능 재도입 (v3.19에서 한 차례 제거했던 것을 이후 버전에서 다시 구현)
- 피드·기도 탭 기간 필터 주별 디폴트 적용, 주별 네비게이터 추가
- 알림 버튼 켜짐/꺼짐 시각 표시, 알림 시간 표시 너비 수정
- 오프라인 캐시 install 이벤트 수정 (⚠️ sw.js에 `install` 리스너 래퍼가 누락되어 있던 문법 오류를 재수정함)
- ※ 이 구간의 세부 변경 이력은 정리되지 않아 위 내용은 코드 diff 기준 요약입니다.
