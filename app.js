...
// ══════════════════════════════════════════
// 배열 정규화 — Firebase 숫자키 객체 → 배열
// (loadHistoryFromCloud, startUserHistoryListener, importData 공통 사용)
// ══════════════════════════════════════════
...
async function loadHistoryFromCloud() {
  ...
  try {
    ...
    Object.keys(cloudData).forEach(k => {
      ...
      if (cTs > lTs) merged[k] = { ...cloudData[k], gratitude: normalizeGratitude(cloudData[k].gratitude) };
    });
    saveHistory(merged);
...
function startUserHistoryListener() {
  ...
  userListener = userRef.on("value", snap => {
    ...
    Object.keys(cloudData).forEach(k => {
      ...
      const lTs = (local[k] && local[k].updatedAt) || 0;
      if (cTs > lTs) {
        local[k] = { ...cloudData[k], gratitude: normalizeGratitude(cloudData[k].gratitude) };
        changed = true;
      }
    });
    if (changed) {
...
// 연속일 캐시 (날짜가 바뀌거나 기록이 저장될 때 무효화)
let _streakCache = null;
let _streakCacheDate = "";
...
function calcStreak() {
  ...
  if (_streakCache !== null && _streakCacheDate === todayStr) return _streakCache;
  const h = getHistory();
  let streak = 0;
...
function saveNickname() {
  ...
  updateProfileBtn();
  // 이름 변경 vs 신규 가입 vs 동일 이름 구분 → 토스트 1번만
...
async function confirmLeaveGroup() {
  ...
  try {
    // Firebase 클라우드 데이터 삭제 (내 기록 + 기도노트)
    if (firebaseReady) {
      const nick = getNickname();
      if (nick && db) {
        const enc = encodeNick(nick);
        try { await db.ref(`grateful-users/${enc}`).remove(); } catch(e) { console.warn("클라우드 데이터 삭제 실패:", e); }
      }
    }
...
function initPwa() {
  ...
  window.addEventListener('beforeinstallprompt', e => {
    ...
    const isDismissed = dismissed && Date.now() < Number(dismissed);
    if (!isDismissed) {
  ...
  const dismissedVal = localStorage.getItem('grateful-pwa-dismissed');
  const isDismissed = dismissedVal && Date.now() < Number(dismissedVal);
...
function _showInstallBanner(platform) {
  const dismissed = localStorage.getItem('grateful-pwa-dismissed');
  if (dismissed && Date.now() < Number(dismissed)) return; // 7일 이내 닫은 경우 재표시 안 함
...
function showChallengeModal() {
  // 기존 오버레이 제거 (중복 방지)
  const existing = document.getElementById("challengeOverlay");
...
function startFeedListener() {
  ...
  query.once("value", snap => {
    ...
    // 이후 실시간 신규 항목 감지 (child_added) — once 완료 후 등록해야 중복 방지
...
function openShareModal() {
  ...
  // 닉네임 표시 + 버튼 레이블에 실명 반영
  const nickLabel = document.getElementById("shareNickLabel");
  if (nickLabel) nickLabel.textContent = `${nick}님으로 공유`;
  const nameShareBtn = document.getElementById("shareNameBtn");
  if (nameShareBtn) nameShareBtn.textContent = `${nick}님으로 공유`;
...
function renderFeed() {
  ...
  authorKeys.forEach(nick => {
    ...
    authorBtns += `<button class="feed-author-btn ${isActive?'active':''}" data-nick="${escHtml(nick)}" onclick="setFeedAuthorFilter(this.dataset.nick)">${escHtml(nick)} ${cnt}</button>`;
  });
  if (anonCount > 0) {
